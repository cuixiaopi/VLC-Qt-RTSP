...
VLCPlayer::VLCPlayer(QObject *parent) : QObject(parent), m_eventManager(nullptr)
{
    ...
    if (!m_pVLC_Inst) {
            qDebug() << "Failed to create VLC instance: " << libvlc_errmsg();
        }
}
...
//rtsp视频解析
int VLCPlayer::Play_RTSP(QString url, uint32_t hwnd)
{
    if (this->IsPlaying()) {
        this->Stop();
    }
    ...
    libvlc_media_t *m;
    // 使用 libvlc_media_new_location 而不是 libvlc_media_new_path 来加载网络流
    m = libvlc_media_new_location(m_pVLC_Inst, url.toStdString().data());
    if (nullptr == m) {
        qDebug() << "Could not open the media stream: " << url;
        return -1;
    }
